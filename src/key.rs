...
use crate::{
    ...
    tone::{NeutralTone, ToneVariant, Tone, Interval},
};
...
impl fmt::Display for KeyType {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            KeyType::Ionian => {write!(f, "{}", "Ionian".green().bold().italic())},
            KeyType::Dorian => {write!(f, "{}", "Dorian".blue().bold().italic())},
            KeyType::Phrygian => {write!(f, "{}", "Phrygian".cyan().bold().italic())},
            KeyType::Lydian => {write!(f, "{}", "Lydian".green().bold().italic())},
            KeyType::Mixolydian => {write!(f, "{}", "Mixolydian".yellow().bold().italic())},
            KeyType::Aeolian => {write!(f, "{}", "Aeolian".blue().bold().italic())},
            KeyType::Locrian => {write!(f, "{}", "Locrian".purple().bold().italic())},
        }
    }
}
...
impl Key {
    pub(super) fn new(tonic: Tone, key_type: KeyType) -> Self {
        Key{tonic: tonic.rematch_key(&key_type), key_type}
    }
    ...
    pub(super) fn sample(difficulty: Difficulty) -> anyhow::Result<Self> {
        ...
        let ionian = Key {tonic, key_type: KeyType::Ionian};
    ...
    fn gen_tone(&self, idx: i8) -> Tone {
        let interval = match idx {
            ...
            2 => match self.key_type {
                KeyType::Ionian
                | KeyType::Lydian | KeyType::Mixolydian
                | KeyType::Aeolian | KeyType::Dorian => {
                    Interval::MajorSecond
                },
                KeyType::Phrygian | KeyType::Locrian => {
                    Interval::MinorSecond
                },
            },
            3 => match self.key_type {
                KeyType::Ionian | KeyType::Lydian | KeyType::Mixolydian => {
                    Interval::MajorThird
                },
            ...
            4 => match self.key_type {
                KeyType::Ionian | KeyType::Phrygian
                | KeyType::Mixolydian | KeyType::Aeolian
                | KeyType::Dorian | KeyType::Locrian => {
            ...
            6 => match self.key_type {
                KeyType::Ionian | KeyType::Lydian
                | KeyType::Mixolydian | KeyType::Dorian => {
                    Interval::MajorSixth
                },
                KeyType::Phrygian | KeyType::Aeolian | KeyType::Locrian => {
                    Interval::MinorSixth
                },
            },
            7 => match self.key_type {
                ...
                KeyType::Phrygian | KeyType::Mixolydian
                | KeyType::Aeolian | KeyType::Locrian | KeyType::Dorian => {
    ...
    pub(super) fn change_mode(&self, idx: i8) -> Key {
        ...
        debug!("Key::change_mode(): change {} to {} using idx {}",
            self, new_key, idx
        );
        new_key
    }
